...
2. Ask the user only for missing inputs: mark, jurisdiction/office, Nice class,
   and online-presence preference.
...
5. Use a web-search MCP tool only if the user opted into online search. If no
   web-search MCP is connected, document the gap in Section 3.
...
9. Link the PDF only after the tool confirms the file exists.
